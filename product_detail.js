...
window.onload = async function () {
  ...
  const productTransaction_status = document.getElementById(
  ...
  for (let i = 0; i < response.category.length; i++) {
    ...
    aElement.setAttribute(
      "onclick",
      `categoryFilter(${response.category[i].id})`
    );
  ...
  if (response.images.length == 0) {
    newImage.setAttribute(
      "src",
      `https://img1.daumcdn.net/thumb/R1280x0/?scode=mtistory2&fname=https%3A%2F%2Ft1.daumcdn.net%2Fcfile%2Ftistory%2F99F519345EE1ED620D`
    );
    newImage.setAttribute("class", "img-fluid");
  } else {
    newImage.setAttribute(
      "src",
      `https://lucedude.link/${response.images[0].image}`
    );
...
async function productDelete(product_id) {
  if (confirm("삭제하시겠습니까?")) {
    ...
    const response = await fetch(
      `https://lucedude.link/product/${product_id}`,
      {
        headers: {
